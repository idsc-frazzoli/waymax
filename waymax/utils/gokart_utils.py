...
def create_init_state(num_timesteps=300):
    ...
    the last point of the log trajectory to the last point of the centerline, which will be used to calculate the progress
    ...
    track_control_points = TrackControlPoints()
    sim_trajectory.x = sim_trajectory.x.at[0, 0].set(track_control_points.x[0])
    sim_trajectory.y = sim_trajectory.y.at[0, 0].set(track_control_points.y[0])
    ...
    roadgraph_points, x_center, y_center, cumulative_length = generate_racing_track(
            track_control_points.x,
            track_control_points.y,
            track_control_points.r)
    ...
    return datatypes.GoKartSimState(
            sim_trajectory=sim_trajectory, log_trajectory=trajectory, log_traffic_light=traffic_light,
...
def create_batch_init_state(batch_size=2, num_timesteps=200):
    ...
    metadata = datatypes.ObjectMetadata(ids=jnp.zeros((batch_size, 1), dtype=jnp.int32),
                                        ...
                                        is_controlled=jnp.ones((batch_size, 1),
                                                               dtype=jnp.bool_))  # shape (batch_size, num_objects=1)
