...
def img_from_fig(fig: matplotlib.figure.Figure) -> np.ndarray:
    ...
    img = data.reshape(fig.canvas.get_width_height()[::-1] + (4,))
    plt.close(fig)
...
def get_n_colors(
    ...
    hsv_list = [(x * max_hue / num_color, saturation, lightness) for x in range(num_color)]
...
def plot_numpy_bounding_boxes(
    ...
    if bboxes.ndim != 2 or bboxes.shape[1] != 5 or color.shape != (3,):
        raise ValueError(
            ("Expect bboxes rank 2, last dimension of bbox 5, color of size 3," " got{}, {}, {} respectively").format(
