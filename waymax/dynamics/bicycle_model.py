...
def compute_inverse(
    ...
    Returns:
      An Action that converts traj[timestep] to traj[timestep+1] of shape
        (..., num_objects, dim=2).
    """
    xy_yaw_vel = jnp.stack([traj.x, traj.y, traj.yaw, traj.vel_x, traj.vel_y], axis=-1)
    xy_yaw_vel_slice = jax.lax.dynamic_slice_in_dim(xy_yaw_vel, start_index=timestep, slice_size=2, axis=-2)
    # Each has shape (..., num_timesteps = 2, 1).
    _, _, yaw, vel_x, vel_y = jnp.split(xy_yaw_vel_slice, 5, axis=-1)
    valids = jax.lax.dynamic_slice_in_dim(traj.valid, start_index=timestep, slice_size=2, axis=-1)
    ...
    real_new_yaw = jnp.where(jnp.abs(new_speed) <= _SPEED_LIMIT, new_yaw, real_new_yaw)
