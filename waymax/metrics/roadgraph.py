...
def is_offroad(
    ...
    Args:
      ...
      safety_margin: The extra safety margin to consider when determining if the trajectory is offroad

    Returns:
      is offroad: a bool array with the shape (..., num_objects). The value is
      True if the bbox is offroad.
    """
    distances = compute_signed_distance_object_to_nearest_road_edge_point(trajectory, roadgraph_points)
    # Shape: (..., num_objects).
    return jnp.greater(distances, safety_margin)


def compute_signed_distance_object_to_nearest_road_edge_point(
        trajectory: datatypes.Trajectory,
        roadgraph_points: datatypes.RoadgraphPoints,
) -> jax.Array:
    """
      ...
      Returns:
        distances: A float array with shape (..., num_objects) representing the signed distance to the road
