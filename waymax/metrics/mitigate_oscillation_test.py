...
class MitigateOscillationMetricTest(tf.test.TestCase, parameterized.TestCase):
    def test_init_flag(self):
        ...
        modified_simulator_state = dataclasses.replace(
            simulator_state,
            actions_history = datatypes.SDC_actions_history(
                data=jnp.array([[1.0, 1.0],[-1.0, -1.0]]), valid=jnp.zeros((2,1), dtype=jnp.bool_)
            )
        )
        result = mitigate_oscillation.MitigateOscillationMetric().compute(modified_simulator_state)
        self.assertEqual(modified_simulator_state.timestep, 0)
