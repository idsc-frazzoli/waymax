...
class PlanningAgentDynamics(_dynamics.DynamicsModel):
    ...
    def forward(
        ...
        """
        # (..., action_dim) --> (..., num_objects, action_dim).
        tiled_action_data = jnp.repeat(action.data[..., jnp.newaxis, :], trajectory.num_objects, axis=-2)
        tiled_valid = jnp.repeat(action.valid[..., jnp.newaxis], trajectory.num_objects, axis=-1)
    ...
    def inverse(
        ...
        """
        multi_agent_action = self.wrapped_dynamics.inverse(trajectory, metadata, timestep)
        return datatypes.select_by_onehot(multi_agent_action, metadata.is_sdc, keepdims=False)
...
class PlanningAgentEnvironment(abstract_environment.AbstractEnvironment):
    ...
    def __init__(
        ...
        if config.controlled_object != _config.ObjectType.SDC:
            raise ValueError(
                f"controlled_object {config.controlled_object} must be SDC for" " planning agent environment."
            )
        ...
        if len(self._sim_agent_actors) != len(self._sim_agent_params):
            raise ValueError("Number of sim agents must match number of sim agent params.")
    ...
    def metrics(self, state: PlanningSimState) -> types.Metrics:
        ...
        multi_agent_metrics_names = (
            "log_divergence", "overlap", "offroad", "sigmoid_log_divergence_2.5", "get_target_2.5", "penalize_headback", "penalize_large_deviation"
        )
    ...
    def step(
        ...
        for agent, actor_state, params, key in zip(
            ...
            merged_action_data = jnp.where(controlled_by_sim[..., jnp.newaxis], action.data, merged_action.data)
            merged_action_valid = jnp.where(controlled_by_sim[..., jnp.newaxis], action.valid, merged_action.valid)
            merged_action = datatypes.Action(data=merged_action_data, valid=merged_action_valid)
...
def _initialized_overlap(log_trajectory: datatypes.Trajectory) -> jax.Array:
    ...
    """
    trajectory = datatypes.dynamic_index(log_trajectory, 0, axis=-1, keepdims=False)
