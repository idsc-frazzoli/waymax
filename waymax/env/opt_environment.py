...
class OptimizationEnvironment(abstract_environment.AbstractEnvironment):
    """Optimization Waymax environment for multi-agent scenarios.
    Differs from the Base Environment solely in the fidelity of the dynamics forward update."""
    ...
    def metrics(self, state: datatypes.SimulatorState) -> types.Metrics:
        ...
        return metrics.run_metrics(simulator_state=state, metrics_config=self.config.metrics)
    ...
    def reset(self, state: datatypes.SimulatorState, rng: jax.Array | None = None) -> datatypes.SimulatorState:
        ...
        return datatypes.update_state_by_log(state_uninitialized, self.config.init_steps)
    ...
    def step(
        ...
        new_traj = state.sim_trajectory
        ts, ts_next = state.timestep, state.timestep + 1
        for _ in range(2):
            ...
            # Shift x, y, velocity, and yaw values
            new_traj = new_traj.replace(
                **{
                    attr: getattr(new_traj, attr).at[:, ts].set(getattr(new_traj, attr)[:, ts_next])
                    for attr in ["x", "y", "z", "vel_x", "vel_y", "yaw"]
                },
            )

        # Restore original trajectory values for the timestep
        new_traj = new_traj.replace(
            **{
                attr: getattr(new_traj, attr).at[:, ts].set(getattr(state.sim_trajectory, attr)[:, ts])
                for attr in ["x", "y", "z", "yaw", "vel_x", "vel_y"]
            }
        )
    ...
    def action_spec(self) -> datatypes.Action:
        ...
        valid_spec = specs.Array(shape=(self.config.max_num_objects, 1), dtype=jnp.bool_)
...
def _get_control_mask(state: datatypes.SimulatorState, config: _config.EnvironmentConfig) -> jax.Array:
    """Gets the control mask for a multi-agent environment."""
    if config.controlled_object == _config.ObjectType.VALID and not config.allow_new_objects_after_warmup:
        ...
        return datatypes.get_control_mask(state.object_metadata, config.controlled_object)
